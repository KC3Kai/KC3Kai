...
(function(){
	...
	window.KC3SortieManager = {
		...
		advanceNode :function( nodeData, UTCTime ){
			...
			// since event spring 2025 (E60-2 T), enforced landing, TP offered by tanks instead of drum/daihatsu?
			// api_event_id = 9
		...
		checkTaihaShips :function(){
			...
			KC3SortieManager.getSortieFleet().forEach((fleetId, fleetIdx) => {
				...
				fleet.ship().forEach((ship, slotIdx) => {
					...
					if (ConfigManager.alert_taiha_unlock && !ship.lock
						// and without any locked equipment
						&& ship.equipment(true).every(g => !g.lock)) { return; }
