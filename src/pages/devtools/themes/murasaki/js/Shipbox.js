/**
 * Shipbox.js
 * KC3改 Ship Box for Murasaki theme
 */
(function(){
	"use strict";
	
	window.KC3MurasakiShipbox = function( base, rosterId, position,
	...
	KC3MurasakiShipbox.prototype.commonElements = function( isCombinedEscort ){
		...
		$(".ex_item", this.element).toggleClass("item_being_used",
			ConfigManager.info_battle && (this.dameConConsumed.pos === 0 ||
				// Although starshell not equippable in ex-slot for now
		...
		if(tagColorId > 0){
			...
			$(".locktag", this.element).show()
				...
				.css("border-color", ConfigManager.pan_ship_icon_border_murasaki);
		} else {
	...
	---------------------------------------------------*/
	KC3MurasakiShipbox.prototype.showHP = function(){
		// HP text
	...
	---------------------------------------------------*/
	KC3MurasakiShipbox.prototype.hideAkashi = function(){
		if(!this.shipData.akashiMark)
	...
	---------------------------------------------------*/
	KC3MurasakiShipbox.prototype.showPrediction = function(){
	...
	---------------------------------------------------*/
	KC3MurasakiShipbox.prototype.showEquipment = function( slot ){
		var thisGear = false;
		if(this.shipData.slotnum > slot){
			...
			if(this.shipData.items[slot] > 0){
				...
				// Check damecon, starshell or rosaK2 or smoke if prediction is enabled
