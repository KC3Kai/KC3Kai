	...
	KC3StrategyTabs.lscs.definition = {
		...
		execute :function(){
			...
			KC3Database.uniquekeys_lscs("flag", keys => {
				...
				$(".secretary_ship").val(this.filters.flagship || 0);
			});
			...
			$(".filters .build_devmat").on("change", function(e) {
				self.filters.devmat = Number($(this).val());
				self.showList();
			});
		...
		showList :function(){
			...
			KC3Database.count_lscs((r => true), total => {
				...
				if(this.totalItems > 0){
					...
					KC3Database.count_lscs(this.filterFunc, filtered => {
		...
		showPage :function(pageNumber){
			...
			$(".build_list").empty();
			KC3Database.get_lscs(this.filterFunc, pageNumber, this.itemsPerPage, resultArray => {
