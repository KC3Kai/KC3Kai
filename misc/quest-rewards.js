...
fetch("https://kcrdb.hitomaru.dev/quests/data?is_mod=false&sort=api_no").then((resp) => (resp.ok && resp.json())).then((o) => { quests=o; dupes=o.items.filter(q => q.api_select_rewards).map(({api_no, api_title, api_select_rewards}) => ({api_no, api_title, api_select_rewards})); console.debug(quests, dupes); })
...
// check fixed rewards
fetch("https://kcrdb.hitomaru.dev/quests?is_mod=false&sort=api_no&extend=clearItems").then(resp => (resp.ok && resp.json())).then(o => { quests=o; dupes=o.items.filter(q => q.clearItems && q.clearItems.length > 0); console.debug(quests, dupes); })
...
fetch("https://kcrdb.hitomaru.dev/quests/data?is_mod=false&has_api_select_rewards=false&sort=api_no").then(resp => (resp.ok && resp.json())).then(o => { quests=o; qids=o.items.map(q => q.api_no); rids=qids.filter((n,i) => i>0 && n==qids[i-1]); console.debug(quests.total, rids); });
