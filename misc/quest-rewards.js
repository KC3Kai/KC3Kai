...
rewards={}; dupes.map(q => q.select.map(g => g.map(o => (((o.api_kind==11 ? KC3Meta.shipNameById(o.api_mst_id) : o.api_kind==12 ? KC3Meta.gearNameById(o.api_mst_id) : o.api_kind==13 ? KC3Meta.useItemName(o.api_mst_id) : o.api_kind==14 ? KC3Master.furniture(o.api_mst_id).api_title :"")||[o.api_kind,o.api_mst_id].join("="))+" x"+o.api_count+(o.api_slotitem_level?" \u2605+"+o.api_slotitem_level:"") )))).forEach((r,i) => { rewards[dupes[i].id] = r}); console.debug(rewards); copy(JSON.stringify(rewards,null,"\t"))

// check id diff
diff=quests.items.filter(q => (!KC3Meta.quest(q.api_no))); meta={}; diff.forEach(q => { meta[q.api_no]={ code:"???", name:q.api_title, desc:q.api_detail, memo:"※", hash:q.api_title.hashCode()}; }); console.debug(diff.length, meta); copy(JSON.stringify(meta,null,"    "))

// check text diff
diff=quests.items.filter(q => { m = KC3Meta.quest(q.api_no); return m.name != q.api_title || m.desc.replace(/<br>/g,'') != q.api_detail.replace(/<br>/g,''); }).map(q => ({id: q.api_no, name: q.api_title, desc: q.api_detail})); console.debug(diff); copy(JSON.stringify(diff,null,"    "))
